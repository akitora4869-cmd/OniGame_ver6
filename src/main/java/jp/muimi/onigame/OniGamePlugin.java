...
public final class OniGamePlugin extends JavaPlugin implements Listener, CommandExecutor, TabCompleter {
    ...
    private LivingEntity getOniEntity(){if(state==GameState.WAITING&&trainingDummy!=null&&Bukkit.getEntity(trainingDummy) instanceof LivingEntity entity)return entity;if(oni!=null&&Bukkit.getPlayer(oni) instanceof Player human&&human.isOnline()&&human.getGameMode()!=GameMode.SPECTATOR)return human;if(oniBot!=null&&Bukkit.getEntity(oniBot) instanceof LivingEntity entity)return entity;for(UUID proxyId:disconnectOniProxy.values())if(Bukkit.getEntity(proxyId) instanceof LivingEntity proxy&&proxy.isValid()&&!proxy.isDead())return proxy;return null;}
    ...
    private void openOniMenu(Player p){
        ...
        int slotOni=8;
        if(!isOniTypeLocked(OniType.DAKKO)){menu.setItem(slotOni,customModelItem(Material.CHAINMAIL_HELMET,1001,"&5&l堕狐","menu_oni:DAKKO","&7奇襲・攪乱型","&d狐渡り &7/ &6分霊 &7/ &6狐火 &7/ &d天来",shown==OniType.DAKKO?"&a現在表示・選択中":"&eクリックして選択"));slotOni+=2;}
        if(!isOniTypeLocked(OniType.KISHIN)){menu.setItem(slotOni,customModelItem(Material.CARVED_PUMPKIN,1203,"&4&l鬼王 &7(きおう)","menu_oni:KISHIN","&7正面突破・追跡型","&c鬼突 &7/ &4地砕 &7/ &4鬼吼 &7/ &6剛身",shown==OniType.KISHIN?"&a現在表示・選択中":"&eクリックして選択"));slotOni+=2;}
        if(!isOniTypeLocked(OniType.SHIKKI)){menu.setItem(slotOni,customModelItem(Material.CARVED_PUMPKIN,1201,"&c&l疾鬼","menu_oni:SHIKKI","&7跳躍・高速機動・連続狩猟型","&6跳躍狩り &7/ &c血走 &7/ &c狂奔 &7/ &4狩猟連鎖",shown==OniType.SHIKKI?"&a現在表示・選択中":"&eクリックして選択"));slotOni+=2;}
        if(!isOniTypeLocked(OniType.YUUKI)){menu.setItem(slotOni,customModelItem(Material.CARVED_PUMPKIN,1202,"&8&l幽鬼","menu_oni:YUUKI","&7隠密・奇襲型","&8幽歩 &7/ &7朧渡り &7/ &5影縫い &7/ &8神隠し",shown==OniType.YUUKI?"&a現在表示・選択中":"&eクリックして選択"));slotOni+=2;}
        if(!isOniTypeLocked(OniType.KANKI)){menu.setItem(slotOni,customModelItem(Material.TOTEM_OF_UNDYING,0,"&5&lEX鬼 鬼神 &7(オニガミ)","menu_oni:KANKI","&7召喚・指揮型","&5鬼喚び &7/ &d鬼令 &7/ &6再臨 &7/ &4百鬼夜行",shown==OniType.KANKI?"&a現在表示・選択中":"&eクリックして選択"));slotOni+=2;}
        if(!isOniTypeLocked(OniType.JAKUTSUKI)&&slotOni<=17)menu.setItem(slotOni,customModelItem(Material.PLAYER_HEAD,2009,"&8&lEX鬼 蛇窟姫 &7(ジャクツキ)","menu_oni:JAKUTSUKI","&7領域制圧・終盤強化型","&5黒鏡 &7/ &5薙ぎ払い &7/ &8蛇崩 &7/ &4解放 &7/ &5黒の波動",shown==OniType.JAKUTSUKI?"&a現在表示・選択中":"&eクリックして選択"));
